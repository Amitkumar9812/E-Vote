  ...
  <form class="form-inline">
   ...
   <a class="btn btn-outline-primary my-2 my-sm-0" href="DataRetrive.php">Data Retrive</a> 
	...
	<center>
        ...
        $sno=$_SESSION['snoGlobal'];
        $updatepassword=$_REQUEST['Password1'];
		//Hashing Algo
        $options = ['cost' => 12,];
        $hashpass= password_hash($updatepassword, PASSWORD_BCRYPT, $options);

        $sql="UPDATE `login_details` SET `Password`='$hashpass' WHERE Email='$userEmail';";
        $sql1="UPDATE `secret_data` SET `hash`='$hashpass',`Password`='$updatepassword' WHERE sno='$sno';";
